...
const botApp = (config) => {
    ...
    app.use( '/api/slack', slackRouter({ config: config.slack, assistant: app.assistant }) ); // pass the slack config settings to the router in an object
    
    // load Watson API routes
    app.use('/api/watson', watsonRouter({ config: config.watson, assistant: app.assistant }) );
