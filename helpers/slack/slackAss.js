...
function SlackAss({ config, assistant }) {
  ...
  this.isToMe = (e) => {
    ...
    let toMe =
      this.isDM(e) || e.type == 'app_mention' || this.isThread(e) ? true : false
    return toMe
  }
  ...
  this.respondToMessage = async (e) => {
    ...
    this.assistant.getResponse(incoming.message, incoming.userId).then(
      (response) => {
        ...
        response =
          this.isDM(e) || this.isThread(e)
            ? response
            : `<@${incoming.userId}> - ${response}`
        ...
        return slackWebClient.chat.postMessage(outgoing).then(
          (response) => {
