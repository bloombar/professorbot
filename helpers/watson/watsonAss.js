...
function WatsonAss({ config }) {
  ...
  this.getSession = async (userId) => {
    let session =
      this.retrieveSession(userId) || (await this.createSession(userId))
    ...
    return this.assistant
      ...
      .then((res) => {
        ...
        if (
          ...
          (responseBody.intents && responseBody.intents[0].confidence <= 0.5)
        ) {
...
class WatsonSession {
  ...
  updateExpiration() {
    ...
    this.expiration =
      Math.floor(Date.now() / 1000) + parseInt(sessionTimeoutSeconds) // 5 minutes in the future
